...
\section{Commutative comonoid structure}
...
\iftoggle{conf}{
    \begin{definition}
        Let \(\morph{\comonoidtohyp}{\smc{\ccomon}}{\cspdhyp}\) be the prop
    ...
    \begin{definition}
        Let \(\morph{\comonoidtohyp}{\ccomon}{\cspdhyp}\) be the homomorphism
...
\begin{lemma}
    The image of \(\comonoidtohyp\) is in \(\plmcspdhyp\).
\end{lemma}
\iftoggle{proofs}{
    \begin{proof}
        By definition.
...
\begin{lemma}\label{lem:ccomon-term}
    ...
    unique term \(\morph{h}{m}{n} \in \smc{\ccomon}\) up to the axioms of SMCs such
...
\begin{theorem}
    \(\stmcsigma + \smc{\ccomon} \cong \plmcspfihyp\).
\end{theorem}
\begin{proof}
    ...
    \(\smc{\ccomon}\) can be retrieved from this.
\end{proof}
