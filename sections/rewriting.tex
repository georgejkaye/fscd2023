...
Effectively, this means that vertices merged by \(a\) are looking for
\emph{identities} into which we are going to insert one or more edges.
...
If two vertices \(v\) and \(w\) are merged by \(\morph{a}{i + j}{L}\), then
these must not be
...
\begin{lemma}
    ...
    complement \(L^\bot\) as in \cref{def:traced-boundary-complement}.
\end{lemma}
\iftoggle{proofs}{
    \begin{proof}
        ...
        would mean that \(j + m \to L^\bot\) would not be mono, violating the
    ...
    \begin{proof}[Proof (Sketch)]
        ...
        would mean that \(j + m \to L^\bot\) would not be mono.
    \end{proof}
}