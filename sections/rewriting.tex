...
\begin{definition}
    Let \(\foldinterfaces\) be defined
\end{definition}

Note that the image of \(\foldinterfaces\) is not in \(\stmc{\Sigma}\).
...
\iftoggle{proofs}{
    ...
    \begin{proof}
        The \(\onlyifdir\) follows by defining cospans corresponding each part
        of \cref{def:term-rewriting} and composing them together appropriately:
        ...
        The \(\ifdir\) follows by using \cref{lem:traced-decomposition} and the
