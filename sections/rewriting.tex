...
\begin{definition}[\cite{bonchi2021string}]
...
This is not an issue: in \cite{heumuller2011construction}, a method for
enumerating all possible pushout complements for a given rule and matching is
presented.
This means we can inspect each one and choose the rewrite we want.
...
Given a rewrite system \(\mce\), we write \(
    \foldinterfaces[\tracedtosymandfrob{\Sigma}[\mce]]
\) for the map with pointwise action \(
    (l, r)
    \mapsto
    (\foldinterfaces[\tracedtosymandfrob{\Sigma}[l]], \foldinterfaces[\tracedtosymandfrob{\Sigma}[r]])
\).
...
\begin{theorem}\label{thm:traced-rewriting}
    ...
    Then, \(
        ...
        \iltikzfig{strings/category/f}[h][white]
    \) if and only if \(
        \termandfrobtohypsigma[\foldinterfaces[\tracedtosymandfrob{\Sigma}[\iltikzfig{strings/category/f}[g][white]]]]
        \grewrite[\termandfrobtohypsigma[\foldinterfaces[\tracedtosymandfrob{\Sigma}[\mcr]]]]
        \termandfrobtohypsigma[\foldinterfaces[\tracedtosymandfrob{\Sigma}[\iltikzfig{strings/category/f}[h][white]]]].
    \)
\end{theorem}
\iftoggle{proofs}{
    \begin{proof}
