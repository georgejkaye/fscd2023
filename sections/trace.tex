...
the trace by composing the cup and cap, we step back before finishing,
...
\begin{proof}
    ...
    term.
    While there may be many such terms in \(\smc{\Sigma}\), the canonical trace
    being a trace means that any choice of resulting term in
    \(\smc{\Sigma} + \frob\) is equal if and only if the original terms were
    equal in \(\stmc{\Sigma}\).
    Moreover, the additional equations of \(\frob\) do not merge any morphisms
    since the only use of the generators of \(\frob\) in the image of
    \(\tracedtosymandfrob{\Sigma}\) is in the canonical trace, to
    which the Frobenius equations do not apply.
\end{proof}
